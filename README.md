...
1. Download the data from [kaggle](https://www.kaggle.com/datasets/smaranjitghose/corn-or-maize-leaf-disease-dataset).
2. Only keep folders related to maize.
...
4. Open `training/maize-disease-training.ipynb` in Jupyter Notebook.
...
```bash
docker run -t --rm -p 8501:8501 -v C:/Code/maize-disease-classification:/maize-disease-classification tensorflow/serving --rest_api_port=8501 --model_config_file=/maize-disease-classification/models.config
```
...
4. Upload the maize.h5 model in the bucket in the path `models/maize.h5`.
...
2. Create a [Project on GCP](https://cloud.google.com/appengine/docs/standard/nodejs/building-app/creating-project) (Keep note of the project id).
3. Create a [GCP bucket](https://console.cloud.google.com/storage/browser/).
4. Upload the tf .h5 model generate in the bucket in the path `models/maize-model.h5`.
5. Install Google Cloud SDK ([Setup instructions](https://cloud.google.com/sdk/docs/quickstarts)).
6. Authenticate with Google Cloud SDK.
